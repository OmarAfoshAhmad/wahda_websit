/*
  ...
  - Made the column `batch_number` on table `CardIssuanceRegistryAll` required. This step will fail if there are existing NULL values in that column.
  - Made the column `imported_by` on table `FamilyImportArchive` required. This step will fail if there are existing NULL values in that column.
