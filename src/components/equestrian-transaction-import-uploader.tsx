...
import { importEquestrianTransactionsAction, type SkippedRowDetail, type SummaryGroup } from "@/app/actions/import-equestrian-transactions";
...
export function EquestrianTransactionImportUploader({
  companies,
  initialCompanyId,
}: {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        // Run dry-run scan
        const res = await importEquestrianTransactionsAction(base64, purgeOld, true, selectedCompanyId, autoCreateMissing, isAmountNetCompanyShare);
        setAnalysis(res);
        setAnalyzing(false);
      };
  ...
  const handleCommitImport = async () => {
    ...
    try {
      const res = await importEquestrianTransactionsAction(fileBase64, purgeOld, false, selectedCompanyId, autoCreateMissing, isAmountNetCompanyShare);
      setResult(res);
      setImporting(false);
    } catch (err: any) {
      setResult({
      ...
      {!analysis && !result && (
        <Card className="p-6">
          <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
            ...
            <div>
              <h2 className="text-lg font-black text-slate-900 dark:text-white">تحميل وتحليل ملف حركات الفروسية</h2>
          ...
          <div className="space-y-6">
            ...
            {/* File Picker */}
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-700 dark:text-slate-300">ملف حركات الفروسية (.xlsx)</label>
            ...
            <Card className="p-4 border-sky-200 bg-sky-50/20 dark:border-sky-900/30">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500">
                    إذا تم تفعيل هذا الخيار (موصى به للفروسية)، ستقوم المنظومة بحساب الفاتورة الإجمالية عكسياً بناءً على نسبة التغطية المخزنة لكل مستفيد.
                  </p>
                </div>
              </div>
            </Card>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <label htmlFor="purgeOld" className="text-sm font-black text-slate-800 dark:text-white cursor-pointer select-none">
                    مسح جميع حركات الفروسية السابقة الخاصة بهذه الشركة فقط قبل الاستيراد
                  </label>
                  <p className="text-xs text-slate-500">
                    إذا قمت بتفعيل هذا الخيار، سيتم حذف كل الحركات المسجلة لعيادات الفروسية (`EQUESTRIAN`) الخاصة بالشركة المحددة في المنظومة قبل حفظ الحركات المرفوعة حديثاً.
                  </p>
                </div>
              </div>
            </Card>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
              <Link href="/admin/equestrian-transactions">
            ...
            {purgeOld && (
              <Card className="p-4 border-amber-300 bg-amber-50/30 dark:border-amber-800/40 mb-6 flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">
                    عند تأكيد الاستيراد، سيقوم النظام بـ **حذف** كافة حركات الفروسية المسجلة سابقاً لهذه الشركة المحددة نهائياً، ثم كتابة حركات ملف Excel الجديد فقط.
                  </p>
                </div>
              </Card>
            )}
          ...
          {result.success && (
            <div className="space-y-6">
              <p className="text-sm text-slate-500">
                {purgeOld
                  ? "تم مسح حركات الفروسية القديمة بالكامل، واستيراد حركات الملف الجديد بنجاح."
                  : "تم استيراد وحفظ حركات ملف Excel بنجاح وتحديث أسقف الفروسية التراكمية للمستفيدين."}
              </p>
