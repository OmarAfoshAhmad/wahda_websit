...
const ACTIVE_LEGACY_STABILIZE_JOB_KEY = "active_legacy_stabilize_job";

type Props = {
  candidateCount: number;
};

export function LegacyWithBatchStabilizeButton({ candidateCount }: Props) {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem(ACTIVE_LEGACY_STABILIZE_JOB_KEY);
    if (stored) {
      setJobId(stored);
      setMessage(`تم استئناف متابعة مهمة تحويل البطاقات القديمة ${stored}.`);
      setProgress(8);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (!status.success || !status.job) {
        setError(status.error ?? "تعذر جلب حالة مهمة تحويل البطاقات القديمة.");
        return;
      }
      ...
      if (job.state === "succeeded") {
        ...
        window.localStorage.removeItem(ACTIVE_LEGACY_STABILIZE_JOB_KEY);
        setJobId(null);
        router.refresh();
        return;
      }
  ...
  const onClick = () => {
    ...
    const confirmed = window.confirm(
      `سيتم تحويل ${candidateCount.toLocaleString("ar-LY")} بطاقة موسومة كقديمة ولها رقم دفعة إلى مستقرة. هل تريد المتابعة؟`
    );
    if (!confirmed) return;
    ...
    startTransition(async () => {
      const queued = await startMaintenanceJobAction({ kind: "stabilize_legacy_with_batch" });
      ...
      window.localStorage.setItem(ACTIVE_LEGACY_STABILIZE_JOB_KEY, queued.job.id);
    });
  };
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className="inline-flex h-9 items-center justify-center rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 text-xs font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-60"
      >
        {isRunning ? (
          <>
            <Loader2 className="ml-1 h-3.5 w-3.5 animate-spin" />
            جاري المعالجة بالخلفية...
          </>
        ) : (
          <>
            <PlayCircle className="ml-1 h-3.5 w-3.5" />
            تحويل القديمة ذات الدفعة إلى مستقرة (خلفية)
          </>
        )}
      </button>
