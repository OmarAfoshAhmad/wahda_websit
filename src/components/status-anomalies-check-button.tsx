...
import { checkStatusAnomaliesAction } from "@/app/actions/balance-health-actions";

export function StatusAnomaliesCheckButton() {
  ...
  const handleCheck = () => {
    ...
    startTransition(async () => {
      const res = await checkStatusAnomaliesAction();
      if (!res.success) {
    ...
    <div className="space-y-2">
      ...
      {count !== null && (
        <p className="rounded-md border border-sky-200 bg-sky-50 px-3 py-2 text-xs font-bold text-sky-700 dark:border-sky-900 dark:bg-sky-950/20 dark:text-sky-400">
          نتيجة الفحص: {count.toLocaleString("ar-LY")} حالة متناقضة.
        </p>
      )}
