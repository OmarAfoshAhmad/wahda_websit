...
export function EquestrianSearchEngine() {
  const {
    searchInput,
    setSearchInput,
    setCardNumber,
      ...
      <Card className="p-2">
        <form onSubmit={handleSearch} className="flex flex-col gap-3 sm:flex-row">
          <div className="relative flex-1" ref={searchBoxRef}>
            <Input
              id="equestrian-beneficiary-search"
              name="equestrian-beneficiary-search"
              aria-label="البحث عن مستفيد في خدمات الفروسية"
              value={searchInput}
              onChange={(e) => {
      ...
      {!beneficiary && recentBeneficiaries.length > 0 && (
        <Card className="p-3">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-xs font-black text-slate-500 dark:text-slate-400">آخر 5 مستفيدين تم اقتطاع فروسية لهم</p>
            <button
              type="button"
