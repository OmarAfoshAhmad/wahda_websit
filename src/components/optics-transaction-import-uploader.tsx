...
import { importOpticsTransactionsAction, type SkippedRowDetail, type SummaryGroup } from "@/app/actions/import-optics-transactions";
...
export function OpticsTransactionImportUploader({
  companies,
  initialCompanyId,
}: {
      ...
      {!analysis && !result && (
        <Card className="p-6">
          <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
            ...
            <div>
              <h2 className="text-lg font-black text-slate-900 dark:text-white">تحميل وتحليل ملف حركات البصريات</h2>
          ...
          <div className="space-y-6">
            ...
            {/* File Picker */}
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-700 dark:text-slate-300">ملف حركات البصريات (.xlsx)</label>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <label htmlFor="purgeOld" className="text-sm font-black text-slate-800 dark:text-white cursor-pointer select-none">
                    مسح جميع حركات البصريات السابقة الخاصة بهذه الشركة فقط قبل الاستيراد
                  </label>
                  <p className="text-xs text-slate-500">
                    إذا قمت بتفعيل هذا الخيار، سيتم حذف كل الحركات المسجلة لعيادات البصريات (`OPTICS`) الخاصة بالشركة المحددة في المنظومة قبل حفظ الحركات المرفوعة حديثاً.
                  </p>
                </div>
              </div>
            </Card>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
              <Link href="/admin/optics-transactions">
            ...
            {purgeOld && (
              <Card className="p-4 border-amber-300 bg-amber-50/30 dark:border-amber-800/40 mb-6 flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">
                    عند تأكيد الاستيراد، سيقوم النظام بـ **حذف** كافة حركات البصريات المسجلة سابقاً لهذه الشركة المحددة نهائياً، ثم كتابة حركات ملف Excel الجديد فقط.
                  </p>
                </div>
              </Card>
            )}
          ...
          {result.success && (
            <div className="space-y-6">
              <p className="text-sm text-slate-500">
                {purgeOld
                  ? "تم مسح حركات البصريات القديمة بالكامل، واستيراد حركات الملف الجديد بنجاح."
                  : "تم استيراد وحفظ حركات ملف Excel بنجاح وتحديث أسقف البصريات التراكمية للمستفيدين."}
              </p>
