...
export function DentalDeductionAction() {
  const {
    beneficiary,
    amount,
    setAmount,
    ...
    return (
      <Card className="p-6 border border-emerald-100 dark:border-emerald-950 bg-emerald-50/20 dark:bg-emerald-950/10 text-center rounded-xl flex flex-col items-center justify-center gap-3 min-h-[300px]">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 max-w-sm">
          تم تسجيل عملية اقتطاع أسنان بقيمة <span className="font-black text-slate-800 dark:text-white">{formatCurrency(amountNum)} د.ل</span> بنجاح للمستفيد {beneficiary.name}
        </p>
        <Button
          variant="outline"
  ...
  return (
    <Card className="p-5 border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-xl shadow-sm space-y-4">
      ...
      <ConfirmationModal
        ...
        description={`سقف ${dentalLabel} المتبقي لـ ${beneficiary.name} لا يكفي لتغطية كامل حصة ${companyName} عن هذه الفاتورة. ستتحمل ${companyName} مبلغ ${formatCurrency(actualCompanyShare)} د.ل فقط (كامل الباقي من السقف)، وسيُحمَّل المؤمَّن نقداً مبلغ ${formatCurrency(actualPatientShare)} د.ل من إجمالي ${formatCurrency(amountNum)} د.ل. تأكد من إبلاغ المؤمَّن والحصول على موافقته قبل المتابعة.`}
