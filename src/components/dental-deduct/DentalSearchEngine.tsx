...
export function DentalSearchEngine() {
  const {
    searchInput,
    setSearchInput,
    setCardNumber,
      ...
      <Card className="p-2">
        <form onSubmit={handleSearch} className="flex flex-col gap-3 sm:flex-row">
          <div className="relative flex-1" ref={searchBoxRef}>
            <Input
              id="dental-beneficiary-search"
              name="dental-beneficiary-search"
              aria-label="البحث عن مستفيد في خدمات الأسنان"
              value={searchInput}
              onChange={(e) => {
      ...
      {!beneficiary && recentBeneficiaries.length > 0 && (
        <Card className="p-3">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-xs font-black text-slate-500 dark:text-slate-400">آخر 5 مستفيدين تم اقتطاع أسنان لهم</p>
            <button
              type="button"
