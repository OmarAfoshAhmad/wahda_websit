...
export function TransactionRollbackButton({ logId, rolledBack }: Props) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [done, setDone] = useState(rolledBack ?? false);
  const [error, setError] = useState<string | null>(null);

  const handleRollback = async () => {
    if (loading || done) return;
    if (!window.confirm("هل أنت متأكد من التراجع عن هذا الاستيراد؟ سيتم حذف الحركات المستوردة واستعادة الأرصدة السابقة.")) return;
  ...
  return (
    <>
      <button
        ...
        title="التراجع عن استيراد الحركات"
      >
