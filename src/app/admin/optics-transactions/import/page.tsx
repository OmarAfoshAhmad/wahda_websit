...
import { OpticsTransactionImportUploader } from "@/components/optics-transaction-import-uploader";
import Link from "next/link";
import { Stethoscope, ArrowRight } from "lucide-react";
import prisma from "@/lib/prisma";

export default async function OpticsTransactionsImportPage({
  searchParams,
}: {
  ...
  if (!session.is_admin) redirect("/admin/optics-transactions");
  ...
  return (
    <Shell facilityName={session.name} session={session}>
      <div className="space-y-6 pb-12">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 dark:border-slate-800 pb-5">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Link
                href={initialCompanyId ? `/admin/optics-services/${initialCompanyId}` : "/admin/optics-transactions"}
              ...
              <h1 className="text-2xl font-black text-slate-900 dark:text-white">
                {initialCompanyId 
                  ...
                  : "استيراد حركات البصريات لشركة محددة"}
              </h1>
            </div>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {initialCompanyId 
                ? "استيراد وتحديث حركات البصريات التاريخية لمنتسبي هذه الشركة المحددة فقط عبر ملف Excel."
        ...
        {/* Content */}
        <OpticsTransactionImportUploader 
          companies={companies} 
