...
import { PhysiotherapyImportUploader } from "@/components/physiotherapy-import-uploader";

export default async function PhysiotherapyServicesPage({
  searchParams,
}: {
  ...
  if (!canAccess) {
    ...
    if (appMode.includes("PHYSIOTHERAPY")) {
      return (
          ...
          <div className="flex items-center justify-center min-h-[50vh]">
            <Card className="p-8 text-center max-w-md w-full border-red-200 bg-red-50 dark:bg-red-900/10 dark:border-red-900/50">
              ...
              <p className="text-red-600 dark:text-red-500 text-sm">
                حسابك لا يملك صلاحية الدخول لخدمات العلاج الطبيعي. يرجى مراجعة مبرمج النظام لتفعيل صلاحية (physiotherapy_services).
              </p>
            </Card>
          </div>
        </Shell>
  ...
  const transactionFilter: any = { is_cancelled: false, service_category: "PHYSIOTHERAPY" };
  if (isFacility) {
  ...
  const physiotherapyCompanies = companies.filter((c) => c.service_policies.length > 0);
  // جميع الشركات (للاستيراد)
  const allCompaniesForImport = companies.map(c => ({ id: c.id, name: c.name, code: c.code }));

  const PHYSIOTHERAPY_COLORS = [
  ...
  return (
    <Shell facilityName={session.name} session={session}>
      <div className="space-y-6 pb-12">
          ...
          <div>
            <div className="mb-1 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl font-black text-slate-900 dark:text-white">خدمات العلاج الطبيعي</h1>
              </div>
              ...
              <div className="mr-1 flex gap-1 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-1 w-fit">
                  <Link
                    href="/admin/physiotherapy-services?tab=companies"
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 px-1.5 py-0.5 rounded-full">
                        {physiotherapyCompanies.length}
                      </span>
                    </div>
                  </Link>
                  {canImport && (
                    <Link
                      href="/admin/physiotherapy-services?tab=import"
                      className={`px-4 py-2 rounded-md text-sm font-bold transition-colors ${
                        activeTab === "import"
                          ? "bg-white dark:bg-slate-800 text-teal-700 dark:text-teal-400 shadow-sm border border-slate-200 dark:border-slate-700"
                          : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200"
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              إدارة مستفيدي شركات التأمين لخدمات العلاج الطبيعي وتطبيق الاقتطاع المالي.
            </p>
          </div>
        </div>
        ...
        {activeTab === "companies" && (
          <div>
            {physiotherapyCompanies.length === 0 ? (
              <Card className="p-12 text-center border-dashed border-2 border-slate-200 dark:border-slate-700">
                ...
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
                  يرجى إضافة شركات التأمين وتعريف سياسات العلاج الطبيعي (PHYSIOTHERAPY) لها من قسم الصيانة.
                </p>
              </Card>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {physiotherapyCompanies.map((company: any, idx) => {
                  const colors = PHYSIOTHERAPY_COLORS[idx % PHYSIOTHERAPY_COLORS.length];
                  ...
                  return (
                    <Link
                      key={company.id}
                      href={`/admin/physiotherapy-services/${company.id}`}
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {ceiling !== null ? (
                          <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded-full bg-white/70 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                            ...
                            سقف {ceiling.toLocaleString("ar-LY")} جلسة
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded-full bg-white/70 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
          ...
          <div>
            {allCompaniesForImport.length === 0 ? (
              ...
              <div className="space-y-4">
                ...
                <PhysiotherapyImportUploader companies={allCompaniesForImport} />
              </div>
            )}
          </div>
        )}
