...
export default async function PhysiotherapyCompanyPrintPage({
  params,
  searchParams,
}: {
  ...
  const canAccess = hasPermission(session, "physiotherapy_services");
  ...
  const company = (await prisma.insuranceCompany.findUnique({
    ...
    include: {
      service_policies: {
        where: { service_type: { code: 'PHYSIOTHERAPY' } },
  ...
  const physiotherapyPolicy = company.service_policies?.[0];
  const ceiling = physiotherapyPolicy && physiotherapyPolicy.ceiling_amount !== null ? Number(physiotherapyPolicy.ceiling_amount) : null;
  ...
  // جلب جميع حركات العلاج الطبيعي غير الملغاة المطابقة للشروط
  ...
  const copay = Math.max(0, 100 - (physiotherapyPolicy ? Number(physiotherapyPolicy.coverage_percent) : 100));
            ...
            <div key={pageIdx} className="print-page">
              ...
              <div className="flex items-start justify-between border-b-4 border-teal-600 pb-3 mb-4">
                ...
                <div className="text-center">
                  <h2 className="text-lg font-black text-teal-800">كشف حركات {getServiceAlias(company, 'PHYSIOTHERAPY', "العلاج الطبيعي")} المخصصة</h2>
                  <p className="text-xs font-bold text-slate-600 mt-0.5">شركة التأمين: {company.name}</p>
                  {copay > 0 && (
                    <p className="text-[10px] font-black text-amber-700 mt-0.5">نسبة التحمل: {copay}% | السقف: {physiotherapyCeiling !== null ? `${physiotherapyCeiling.toLocaleString("ar-LY")} د.ل` : "مفتوح"}</p>
                  )}
                  {(searchQuery || fromDate || toDate) && (
