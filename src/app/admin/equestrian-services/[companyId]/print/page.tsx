...
export default async function EquestrianCompanyPrintPage({
  params,
  searchParams,
}: {
  ...
  const canAccess = hasPermission(session, "equestrian_services");
  ...
  // جلب جميع حركات الفروسية غير الملغاة المطابقة للشروط
  ...
  const copay = Math.max(0, 100 - (equestrianPolicy ? Number(equestrianPolicy.coverage_percent) : 100));
