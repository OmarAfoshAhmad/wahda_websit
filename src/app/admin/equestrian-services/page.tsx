...
import { EquestrianImportUploader } from "@/components/equestrian-import-uploader";

export default async function EquestrianServicesPage({
  searchParams,
}: {
  ...
  // نعرض فقط الشركات التي لها سياسة فروسية مُعرفة
  const equestrianCompanies = companies.filter((c) => c.service_policies.length > 0);
  // جميع الشركات (للاستيراد)
  const allCompaniesForImport = companies.map(c => ({ id: c.id, name: c.name, code: c.code }));

  const EQUESTRIAN_COLORS = [
          ...
          <div>
            <div className="mb-1 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl font-black text-slate-900 dark:text-white">الفروسية</h1>
              </div>
              ...
              <div className="mr-1 flex gap-1 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-1 w-fit">
                  <Link
                    href="/admin/equestrian-services?tab=companies"
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 px-1.5 py-0.5 rounded-full">
                        {equestrianCompanies.length}
                      </span>
                    </div>
                  </Link>
                  {canImport && (
                    <Link
                      href="/admin/equestrian-services?tab=import"
                      className={`px-4 py-2 rounded-md text-sm font-bold transition-colors ${
                        activeTab === "import"
                          ? "bg-white dark:bg-slate-800 text-teal-700 dark:text-teal-400 shadow-sm border border-slate-200 dark:border-slate-700"
                          : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200"
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              إدارة خدمة الطوارئ والعمليات للفروسية وتطبيق الاقتطاع المالي.
            </p>
          </div>
        </div>
          ...
          <div>
            {equestrianCompanies.length === 0 ? (
              <Card className="p-12 text-center border-dashed border-2 border-slate-200 dark:border-slate-700">
                ...
                <h3 className="text-lg font-black text-slate-700 dark:text-slate-300">لا توجد شركات بسياسة فروسية</h3>
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
                  يرجى إضافة شركات التأمين وتعريف سياسات الفروسية (EQUESTRIAN) لها من قسم الصيانة.
                </p>
              </Card>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {equestrianCompanies.map((company: any, idx) => {
                  ...
                  return (
                    <Link
                      key={company.id}
                      href={`/admin/equestrian-services/${company.id}`}
          ...
          <div>
            {allCompaniesForImport.length === 0 ? (
              ...
              <div className="space-y-4">
                ...
                <EquestrianImportUploader companies={allCompaniesForImport} />
              </div>
            )}
          </div>
        )}
