...
function canReadOptics(session: Parameters<typeof hasPermission>[0]) {
  return hasPermission(session, "optics_services") || hasPermission(session, "view_optics_beneficiaries");
}
...
export async function searchCompanyBeneficiaries(query: string, companyId: string) {
  ...
  if (!canReadOptics(session)) return { error: "غير مصرح", items: [] };
  try {
...
export async function getOpticsBeneficiaryDetail(beneficiaryId: string, companyId: string) {
  ...
  if (!canReadOptics(session)) return { error: "غير مصرح" };
