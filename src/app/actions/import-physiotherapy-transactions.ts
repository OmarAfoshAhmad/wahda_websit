...
export async function importPhysiotherapyTransactionsAction(
  ...
  try {
    ...
    if (!ws) {
      return { success: false, error: "ملف Excel فارغ أو لا يحتوي على ورقة عمل صالحة.", totalRows: 0, insertedCount: 0, skippedCount: 0, autoCreatedCount: 0, autoCreatedFacilitiesCount: 0, ceilingExceededCount: 0, ceilingExceededDetails: [], skippedDetails: [], groups: [] };
    }
    ...
    const policyMap = new Map(
      dbCompanies.map((c) => {
        const physiotherapyPolicy = (c as any).service_policies?.find((p: any) => p.service_type?.code === "PHYSIOTHERAPY");
        return [
          c.id,
          {
            service_type: "PHYSIOTHERAPY",
            annual_ceiling: physiotherapyPolicy && physiotherapyPolicy.ceiling_amount !== null ? Number(physiotherapyPolicy.ceiling_amount) : null,
            copay_percentage: Math.max(0, 100 - (physiotherapyPolicy ? Number(physiotherapyPolicy.coverage_percent) : 100)),
    ...
    const groupStats = new Map<string, { companyName: string; facilityName: string; count: number; totalAmount: number; isMatched: boolean; reason?: string }>();
    ...
    // عداد المستفيدين والمرافق الذين أُنشئوا تلقائياً
    let autoCreatedCount = 0;
    let autoCreatedFacilitiesCount = 0;

    // Process rows
    for (const r of rawRows) {
      let facility = resolveFacility(r.facilityName);

      // ── إنشاء المرفق تلقائياً إذا لم يكن موجوداً ──
      if (!facility && r.facilityName && autoCreateMissingFacilities) {
        const cleanFacName = r.facilityName.trim();
        if (dryRun) {
          const tempFac = { id: `__temp_fac__${cleanFacName}`, name: cleanFacName, username: `__temp_usr__${Date.now()}` };
          (dbFacilities as any[]).push(tempFac);
          facility = tempFac;
        } else {
          try {
            // تحقق ربما تم إنشاؤه للتو في صف سابق
            const exists = await prisma.facility.findFirst({ where: { name: cleanFacName, deleted_at: null }});
            if (exists) {
              facility = exists;
            } else {
              const defaultPasswordHash = await bcrypt.hash("123456", 10);
              const generatedUsername = `pt_${Date.now().toString().slice(-6)}_${Math.floor(Math.random() * 1000)}`;
              ...
              (dbFacilities as any[]).push(newFac);
              facility = newFac;
              autoCreatedFacilitiesCount++;
              logger.info(`Auto-created facility: ${cleanFacName}`);
            }
          } catch (err) {
            logger.warn(`Failed to auto-create facility ${cleanFacName}:`, { error: String(err) });
          }
        }
      }
      ...
      if (!groupStats.has(groupKey)) {
        ...
        groupStats.set(groupKey, { companyName, facilityName: resolvedFacilityName, count: 0, totalAmount: 0, isMatched, reason });
      }
      ...
      const idempotencyKey = `import-physiotherapy-tx:${r.rowNumber}:${r.card}:${r.amount}:${dateStr}`;
