...
const FACILITY_MAP: Record<string, string> = {
  // الجهمي
  "الجهمي للفروسية": "waljahmi_eye",
  "شركه مجموعة الجهمي": "waljahmi_eye",
  "مجموعة الجهمي": "waljahmi_eye",
  "مجموعة الجهمي للفروسية": "waljahmi_eye",
  ...
  // ساطي
  "ساطي": "wsati_eye",
  "ساطي للفروسية": "wsati_eye",
  "شركة ساطي": "wsati_eye",
  "الساطي": "wsati_eye",
  "شركة الساطي": "wsati_eye",

  // دلتا
  "دلتا للفروسية": "wdelta_eye",
  "دلتا الفروسية": "wdelta_eye",
  "دلتا": "wdelta_eye",

  // 21 فروسية
  "21 للفروسية": "wtwenty_one_eye",
  "21فروسية": "wtwenty_one_eye",
  "21 فروسية": "wtwenty_one_eye",

  // برنيق
  "البرنيق": "wberniq_eye",
  "البرنيق للفروسية": "wberniq_eye",
  "شركة برنيق الجديد": "wberniq_eye",
  "برنيق الجديد": "wberniq_eye",

  // الرؤية
  "الرؤية للفروسية": "wroaya_eye",
  "رؤية للفروسية": "wroaya_eye",
  "رؤيه": "wroaya_eye",
  "رؤية": "wroaya_eye",

  // الأنيس
  "الانيس للفروسية": "walanis_eye",
  "الانيس": "walanis_eye",
  ...
  // رشاد
  "الرشاد": "wrashad_eye",
  "رشاد للفروسية": "wrashad_eye",
  "مركز رشاد": "wrashad_eye",

  // المتقدم
  "المتقدم": "wmotakadem_eye",
  "المتقدم للفروسية": "wmotakadem_eye",

  // ابن سينا
  "ابن سينا  للفروسية": "wibnsina_eye",

  // بوراوي
  "بوراوي للفروسية": "wborawi_eye",

  // فروسية الشريف
  "فروسية الشريف": "walshareef_eye",

  // أبو النجا
  "ابو النجا": "wabonaja_eye",

  // الوصال
  "الوصال للفروسية": "walwesal_eye",
  "الوصال": "walwesal_eye",
...
export async function importEquestrianTransactionsAction(
  fileBase64: string,
  ...
  if (!session || !hasPermission(session, "equestrian_services")) {
  ...
  try {
    ...
    const policyMap = new Map(
      dbCompanies.map((c) => {
        const equestrianPolicy = (c as any).service_policies?.find((p: any) => p.service_type?.code === "EQUESTRIAN");
        return [
          c.id,
          {
            service_type: "EQUESTRIAN",
            annual_ceiling: equestrianPolicy && equestrianPolicy.ceiling_amount !== null ? Number(equestrianPolicy.ceiling_amount) : null,
            copay_percentage: Math.max(0, 100 - (equestrianPolicy ? Number(equestrianPolicy.coverage_percent) : 100)),
    ...
    if (!dryRun && purgeOld) {
      ...
      logger.info("Purged previous equestrian transactions as requested.");
    }
    ...
    for (const r of rawRows) {
      ...
      if (policy) {
        ...
        if (beneficiary.custom_ceilings && typeof beneficiary.custom_ceilings === "object" && "EQUESTRIAN" in (beneficiary.custom_ceilings as any)) {
          const cVal = (beneficiary.custom_ceilings as any).EQUESTRIAN;
      ...
      const idempotencyKey = `import-equestrian-tx:${r.rowNumber}:${r.card}:${r.amount}:${dateStr}`;
    ...
    if (!dryRun) {
      revalidatePath("/admin/equestrian-transactions");
    }
    ...
    logger.error("Equestrian Import Error:", { error: error.message, stack: error.stack });
    return {
      success: false,
