  ...
  try {
    ...
    return {
      items: rows.map(r => {
        ...
        let physiotherapyCeiling = policy?.ceiling_amount !== null && policy?.ceiling_amount !== undefined ? Number(policy.ceiling_amount) : 3000;
        let hasCustomCeiling = false;
        
        if (r.custom_ceilings && typeof r.custom_ceilings === "object" && "PHYSIOTHERAPY" in (r.custom_ceilings as any)) {
          const cVal = (r.custom_ceilings as any).PHYSIOTHERAPY;
          physiotherapyCeiling = cVal === null ? 99999999 : Number(cVal);
        ...
        return {
          ...
          status: r.status,
          remaining_balance: physiotherapyCeiling,
          total_balance: physiotherapyCeiling,
          hasCustomCeiling,
        };
      })
    };
...
export async function getPhysiotherapyBeneficiaryDetail(beneficiaryId: string, companyId: string) {
  const session = await requireActiveFacilitySession();
  if (!session) {
  ...
  try {
    ...
    if (beneficiary.custom_ceilings && typeof beneficiary.custom_ceilings === "object" && "PHYSIOTHERAPY" in beneficiary.custom_ceilings) {
      ...
      physiotherapyCeiling = cVal === null ? null : Number(cVal);
      hasCustomCeiling = true;
    }

    const dynamicRemaining = physiotherapyCeiling === null ? null : Math.max(0, physiotherapyCeiling - yearlyConsumed);
