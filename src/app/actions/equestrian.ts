...
export async function searchCompanyBeneficiaries(query: string, companyId: string) {
  ...
  if (!canReadEquestrian(session)) return { error: "غير مصرح", items: [] };
  try {
    ...
    return {
      items: rows.map(r => {
        ...
        if (r.custom_ceilings && typeof r.custom_ceilings === "object" && "EQUESTRIAN" in (r.custom_ceilings as any)) {
          const cVal = (r.custom_ceilings as any).EQUESTRIAN;
          equestrianCeiling = cVal === null ? 99999999 : Number(cVal);
        ...
        return {
          ...
          status: r.status,
          remaining_balance: equestrianCeiling,
...
export async function getEquestrianBeneficiaryDetail(beneficiaryId: string, companyId: string) {
  const session = await requireActiveFacilitySession();
  if (!session) {
    return { error: "غير مصرح" };
  }
  if (!canReadEquestrian(session)) return { error: "غير مصرح" };
  ...
  try {
    ...
    let hasCustomCeiling = false;
    if (beneficiary.custom_ceilings && typeof beneficiary.custom_ceilings === "object" && "EQUESTRIAN" in beneficiary.custom_ceilings) {
      const cVal = (beneficiary.custom_ceilings as any).EQUESTRIAN;
      equestrianCeiling = cVal === null ? null : Number(cVal);
      hasCustomCeiling = true;
    }
    ...
    logger.error("Get equestrian beneficiary detail error", { error: String(error) });
