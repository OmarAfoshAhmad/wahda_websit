...
async function main() {
  ...
  const outDir = opts.outDir
    ...
    : path.resolve(process.cwd(), "exports", `batch_rebuild_no_additions_${stamp}`);
  ...
  for (const batch of sortedBatches) {
    ...
    for (const r of allRows) {
      ...
      const row = ws.addRow({
        ...
        origin: `الملف الأصلي المجمّع | ${r.source_sheet} | ${r.source_row}`,
      });
    ...
    console.log(`[done] batch=${batch} | originals=${originalCount} | added=0(disabled) | file=${outFile}`);
  }
